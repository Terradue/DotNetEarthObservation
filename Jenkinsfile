  ...
  stages {
    ...
    stage('Package') {
      steps {
        parallel(
          "Package": {
            sh "nuget4mono -g ${env.BRANCH_NAME} -p Terradue.Metadata.EarthObservation/packages.config Terradue.Metadata.EarthObservationh/bin/Terradue.Metadata.EarthObservation.dll Terradue.Metadata.EarthObservation/Resources/**/*,content/Resources"
            sh 'cat *.nuspec'
