...
namespace Terradue.Metadata.EarthObservation.OpenSearch
{
    public class AtomEarthObservationFactory
    {
        ...
        public static AtomItem CreateAtomItemFromEarthObservationType(Terradue.ServiceModel.Ogc.Eop21.EarthObservationType eo)
        {
            ...
            else if (eo is Terradue.ServiceModel.Ogc.Alt21.AltEarthObservationType)
            {

                Terradue.ServiceModel.Ogc.Alt21.AltEarthObservationType altEo = (Terradue.ServiceModel.Ogc.Alt21.AltEarthObservationType)eo;
                var item = CreateAtomItemFromAltEarthObservationType(altEo);
                AddGeoRss(altEo, item);
                return item;
        ...
        public static AtomItem CreateAtomItemFromAltEarthObservationType(Terradue.ServiceModel.Ogc.Alt21.AltEarthObservationType altEo)
        {
            ...
            try
            {
                ...
                item = new AtomItem(String.Format("{0} {1} {2} {3} {4} {5}-{6}",
                                                  altEo.procedure.Eop21EarthObservationEquipment.platform.Platform.shortName,
                                                  altEo.EopMetaDataProperty.EarthObservationMetaData.productType,
                                                  string.Join("/", altEo.procedure.Eop21EarthObservationEquipment.sensor.Sensor.operationalMode.Text),
                                                  altEo.EopMetaDataProperty.EarthObservationMetaData.processing.First().ProcessingInformation.processingLevel,
                                                  altEo.procedure.Eop21EarthObservationEquipment.acquisitionParameters.Acquisition.wrsLongitudeGrid.Value,
                                                  DateTime.Parse(altEo.phenomenonTime.GmlTimePeriod.beginPosition.Value).ToUniversalTime().ToString("yyMMddTHHmmss"),
                                                  DateTime.Parse(altEo.phenomenonTime.GmlTimePeriod.endPosition.Value).ToUniversalTime().ToString("yyMMddTHHmmss")
                                                 ),
                                    "",
                                    null,
                                    altEo.EopMetaDataProperty.EarthObservationMetaData.identifier,
                                    DateTime.Parse(altEo.phenomenonTime.GmlTimePeriod.endPosition.Value)
                                   );



                item.Summary = new Terradue.ServiceModel.Syndication.TextSyndicationContent(
                    GetHtmlSummaryFromAltEarthObservationType(altEo),
                    Terradue.ServiceModel.Syndication.TextSyndicationContentKind.Html);

                MemoryStream stream = new MemoryStream();
                OgcHelpers.Alt21Serializer.Serialize(stream, altEo);
                stream.Seek(0, SeekOrigin.Begin);

                item.ElementExtensions.Add(XmlReader.Create(stream));

                item.ElementExtensions.Add("date", OgcHelpers.DC, string.Format("{0}/{1}", altEo.phenomenonTime.GmlTimePeriod.beginPosition.Value, altEo.phenomenonTime.GmlTimePeriod.endPosition.Value));

                item.ElementExtensions.Add("identifier", OgcHelpers.DC, altEo.EopMetaDataProperty.EarthObservationMetaData.identifier);
        ...
        public static string GetHtmlSummaryFromAltEarthObservationType(Terradue.ServiceModel.Ogc.Alt21.AltEarthObservationType optEo)
        {
